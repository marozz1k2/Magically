...
import { useCreateTtModel, useUpdateTtModel } from "@/hooks/useTtapi";
...
interface CreateTtModelDialogProps {
    open: boolean;
    onOpenChange: (open: boolean) => void;
    modelToEdit?: any;
}

export const CreateTtModelDialog = ({ open, onOpenChange, modelToEdit }: CreateTtModelDialogProps) => {
    const t = useTranslations("Pages.Models.Dialog");
    const tAlerts = useTranslations("Pages.Models.Alerts");

    const createTtModel = useCreateTtModel();
    const updateTtModel = useUpdateTtModel();
    ...
    const onSubmit = async (values: FormValues) => {
        ...
        try {
            ...
            if (isEditing && modelToEdit) {
                await updateTtModel.mutateAsync({ id: modelToEdit.id, formData });
                toast.success(tAlerts("updated"));
            } else {
                await createTtModel.mutateAsync(formData);
    ...
    const isLoading = createTtModel.isPending || updateTtModel.isPending;
